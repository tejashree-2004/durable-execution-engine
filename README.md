...
Architecture
Workflow Code ↓ DurableContext (step engine) ↓ StepStore (Persistence API) ↓ SQLite Storage

Each workflow step is:

Identified with a deterministic step key
Checked against persistent storage
Skipped if already completed
Executed otherwise
Result serialized and stored
...
T step(String id, Callable fn, Class type)
...
steps( workflow_id TEXT, step_key TEXT, status TEXT, output TEXT )
...
create-user-0 laptop-1 access-2 email-3
...
Testing Durability
Run application
Force crash during a step
Restart program
Observe skipped steps
...
Author
Tejashree Software Engineer Intern Assignment
